...
pub fn xxh32(data: &[u8], seed: u32) -> u32 {
    ...
    if len >= 16 {
        ...
        while len - p >= 16 {
            acc1 = round(acc1, u32::from_le_bytes(data[p..p + 4].try_into().unwrap()));
            p += 4;
            acc2 = round(acc2, u32::from_le_bytes(data[p..p + 4].try_into().unwrap()));
            p += 4;
            acc3 = round(acc3, u32::from_le_bytes(data[p..p + 4].try_into().unwrap()));
            p += 4;
            acc4 = round(acc4, u32::from_le_bytes(data[p..p + 4].try_into().unwrap()));
            p += 4;
        }
        h32 = acc1
            .rotate_left(1)
    ...
    while len - p >= 4 {
        h32 = h32.wrapping_add(
            u32::from_le_bytes(data[p..p + 4].try_into().unwrap()).wrapping_mul(PRIME3),
        );
        p += 4;
